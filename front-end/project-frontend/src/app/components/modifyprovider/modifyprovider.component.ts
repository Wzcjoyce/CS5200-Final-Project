...
@Component({
  selector: 'app-modifyprovider',
  templateUrl: './modifyprovider.component.html',
  styleUrls: ['./modifyprovider.component.css']
})
export class ModifyproviderComponent implements OnInit {
  ...
  ModifyProvider()
  {
    ...
    const Provider_id : string = (<HTMLInputElement>document.getElementById("modifyproviderid")).value;
    ...
    if(Provider_id && CompanyName && type && teamSize && quotedPrice && OfficialWebsite && FrontEnd && BackEnd && Data_analysis
      && UI && Logo && Illustration)
    { 
      try{
        this.ProviderService.ModifyProviderFromInput(Provider_id, CompanyName, type, teamSize, quotedPrice, OfficialWebsite,
